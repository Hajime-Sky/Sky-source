...
- `009_storage_migrations.js`: one-time migration from legacy Keychain/local image storage to unified file storage. It is listed under `manifest.json` `migrations`, not normal `parts`; the loader fetches it only when its ID has not been applied, records the applied ID, and deletes any local migration file after loading.
...
- `none`: do not check for updates unless required files are missing or the manual update button is pressed.
