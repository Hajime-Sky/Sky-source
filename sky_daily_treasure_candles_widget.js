...
const BASE_SKY_YMD = "2026-02-15";
const BASE_REALM_INDEX = 4;
const BASE_ROT_INDEX_BY_REALM = { 4: 0 };
const IMAGE_BASE_URL = "https://Hajime-Sky.github.io/Sky-source/treasure-candles-images/";
const CACHE_META_KEY = "sky_daily_treasure_candles_image_cache_meta_v3";
const CACHE_VERSION = 4;
const MIN_SOURCE_IMAGE_SIZE = 1280;
const AUTOMATION_WARN_STATE_KEY = "sky_daily_treasure_candles_warn_state_v2";
const REALMS = [
...
function calcBySkyYMD(targetSkyYMD) {
  const baseDay = dayNumberFromYMD(BASE_SKY_YMD);
  const targetDay = dayNumberFromYMD(targetSkyYMD);
  const offset = targetDay - baseDay;
  const realmIndex = mod(BASE_REALM_INDEX + offset, REALMS.length);
  const realm = REALMS[realmIndex];
  const patterns = PATTERN_DATA[realm.key].patterns;
  const rotCount = patterns.length;
  const a = mod(realmIndex - BASE_REALM_INDEX, REALMS.length);
  const m = (offset - a) / REALMS.length;
  const occ = Number.isFinite(m) ? Math.floor(m) : 0;
  const baseRot = BASE_ROT_INDEX_BY_REALM[realmIndex] ?? 0;
  const rotIndex = mod(baseRot + occ, rotCount);
...
function calcForCurrentLATime(dateObj = new Date()) {
  ...
  const result = calcBySkyYMD(ctx.skyYMD);
  return {
    ...result,
...
function getImagesDir() {
  ...
  const dir = fm.joinPath(fm.documentsDirectory(), "sky_daily_treasure_candles_images_v3");
...
function cleanupOldCaches() {
  const oldPrefix = "sky_daily_treasure_candles_image_cache_v1_";
  const labels = getExpectedLabels();
  for (const label of labels) {
    const key = oldPrefix + label;
    if (Keychain.contains(key)) Keychain.remove(key);
  }
  if (Keychain.contains("sky_daily_treasure_candles_image_cache_meta_v1")) Keychain.remove("sky_daily_treasure_candles_image_cache_meta_v1");
  if (Keychain.contains("sky_daily_treasure_candles_image_cache_meta_v2")) Keychain.remove("sky_daily_treasure_candles_image_cache_meta_v2");
  const fm = FileManager.local();
  const v2Dir = fm.joinPath(fm.documentsDirectory(), "sky_daily_treasure_candles_images_v2");
  if (fm.fileExists(v2Dir)) fm.remove(v2Dir);
}
cleanupOldCaches();
...
function readCacheMeta() {
  if (!Keychain.contains(CACHE_META_KEY)) return null;
  try {
    const parsed = JSON.parse(Keychain.get(CACHE_META_KEY));
...
function writeCacheMeta(meta) {
  Keychain.set(CACHE_META_KEY, JSON.stringify(meta));
}
...
function needsQualityRefresh(meta = readCacheMeta()) {
  ...
  if (!meta) return true;
  if (Number(meta.version || 0) < CACHE_VERSION) return true;
...
function readAutomationWarnState() {
  if (!Keychain.contains(AUTOMATION_WARN_STATE_KEY)) return null;
  try {
    return JSON.parse(Keychain.get(AUTOMATION_WARN_STATE_KEY));
  } catch (e) {
    return null;
  }
}
function writeAutomationWarnState(laKey, laTime) {
  try {
    Keychain.set(AUTOMATION_WARN_STATE_KEY, JSON.stringify({ laKey, laTime, warnedAt: new Date().toISOString() }));
  } catch (e) {}
}
...
function getMinutesFromLaMidnight(la) {
  return (Number(la.hour) * 60) + Number(la.minute);
}
...
async function resetCacheAndReport() {
  if (Keychain.contains(CACHE_META_KEY)) Keychain.remove(CACHE_META_KEY);
  removeAllImageFiles();
  const res = calcForCurrentLATime();
