...
function getServerWallClockDate(dateObj = new Date()) {
  ...
  return new Date(Date.UTC(la.year, la.month - 1, la.day, la.hour, la.minute, la.second));
}
...
function calcByServerDate(serverDate, targetSkyYMD) {
  ...
  const rotIndex = mod(Math.floor((serverDate.getUTCDate() - 1) / 5), patterns.length);
...
function getImagesDir() {
  ...
  const dir = fm.joinPath(fm.documentsDirectory(), "sky_daily_treasure_candles_images");
